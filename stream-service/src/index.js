...
app.get('/messages/:messageId', async (req, res) => {
  try {
    ...
    const messages = await redis.xrange(STREAM_NAME, '-', '+', 'COUNT', 1000);
...
app.get('/stats', async (req, res) => {
  try {
    ...
    const pendingInfo = await redis.xpending(STREAM_NAME, 'booking-workers');
...
app.get('/messages', async (req, res) => {
  try {
    ...
    const messages = await redis.xrange(STREAM_NAME, '-', '+', 'COUNT', count);
